...
<div class="flex justify-center h-screen pt-48 pb-64 font-rubik">
                        ...
                        @if ($wishcheck->us_ID == $userDetails->us_ID && $wishcheck->g_ID == $mygood->g_ID)
                            @if (!$messageDisplayed)
                                <div class="text-2xl mt-8 mb-4 text-[#F12E52] flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-[#F12E52] mr-2"
                                    ...
                                    <span class="font-bold text-sm text-gray-600 mx-4">{{ $userDetails->us_name }}
                                        also wishlisted your goods!</span>
                                </div>
                                @php
                ...
                <div class="flex flex-wrap">
                    @foreach ($wishlist as $wishcheck)
                        @foreach ($myGoods as $mygood)
                            @if ($wishcheck->us_ID == $userDetails->us_ID && $wishcheck->g_ID == $mygood->g_ID)
                                <div
                                    class="max-w-md rounded overflow-hidden shadow-lg product-card flex flex-col mr-4 mb-4">
                                    @php
                                        ...
                                        $imageUrl = isset($images[0])
                                            ...
                                            : $defaultImageUrl;
                                        $formattedPrice =
                                            'Rp ' . number_format($mygood->g_price_prediction, 0, ',', '.');
                                    @endphp

                                    <img class="w-full h-64 object-cover object-center" src="{{ $imageUrl }}"
                                        alt="Product Image" data-product-image="{{ $imageUrl }}">
                                    <div class="px-4 py-4">
                                        ...
                                        <div
                                            class="grid grid-cols-1 gap-2 mt-3 bg-pink-100 border border-gray-300 rounded-lg p-4">
                                            <div>
                                                ...
                                                <span
                                                    class="hidden product-price">{{ $mygood->g_price_prediction }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            @else
                <div class="flex justify-center">
                    <div class="text-xl text-[#F12E52] mt-4 flex items-center">
                        ...
                        <span>You don't hav ematched wishlist items with {{ $userDetails->us_name }}</span>
                    </div>
                </div>
            @endif
    ...
    @if ($similarProducts->count() > 0)
        <div class="text-2xl mt-8 mb-4 text-[#F12E52]"><b>Similar Products</b><span
                class="font-bold text-sm text-gray-600 mx-4">based on Similar Price</span></div>
            ...
            @foreach ($similarProducts as $similarProduct)
                ...
                <div class="max-w-md rounded overflow-hidden shadow-lg product-card">
                    ...
                    <img class="w-full h-64 object-cover object-center"
                        src="{{ $similarProduct->$imageUrl }}" alt="Product Image"
                        data-product-image="{{ $similarProduct->$imageUrl }}">
                    <div class="px-4 py-4">
                        <input type="hidden" id="goods_owner" value="{{ $product->us_ID }}" />
                        ...
                        <div
                            ...
                            <div>
                                ...
                                <span
                                    class="hidden product-price">{{ $similarProduct->g_price_prediction }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <a href="{{ route('goods_detail', ['hashed_id' => Hashids::encode($similarProduct->g_ID)]) }}"
                        ...
                        data-product-id="{{ $product->g_ID }}"
                        data-product-name="{{ $similarProduct->g_name }}"
                        ...
                        data-product-type="{{ $similarProduct->g_type }}"
